...
class DateTime {
    ...
    fun subtractOf(dateTime: DateTime) {
        ...
        } else if (month == 0 || month == 2 || month == 4 || month == 6 || month == 7 || month == 9 || month == 11) multiple =
            31
    ...
    fun toAboutValueNoDot(): String {
        val strTime = StringBuilder()
        if (year > 0) strTime.append(String.format("%d年", year)) else if (month > 0) strTime.append(
            String.format("%d月", month + 1)
        ) else if (day > 0) strTime.append(
            String.format(
                "%d日",
                day + 1
            )
        ) else if (hour > 0) strTime.append(
            String.format(
                "%d时",
                hour
            )
        ) else if (minute > 0) strTime.append(
            String.format(
                "%d分",
                minute
            )
